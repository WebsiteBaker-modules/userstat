...
if(!isset($_GET['list'])) {
	$sql = "SELECT * FROM `" . TABLE_PREFIX . "users` ORDER BY `active` DESC, `display_name`";
} else {
	switch ($_GET['list']) {
		case 'active':
			$sql = "SELECT * FROM `" . TABLE_PREFIX . "users` ORDER BY `active` DESC, `display_name`";
		...
		case 'user':
			$sql = "SELECT * FROM `" . TABLE_PREFIX . "users` ORDER BY `display_name`";
			break;
			
		case 'last_check':
			$sql = "SELECT * FROM `" . TABLE_PREFIX . "users` ORDER BY `login_when` DESC, `display_name`";
			break;
...
if(!$row['active']) {
	($i % 2 == 0) ? $TR_CLASS='class="inactive"' : $TR_CLASS='class="even_inactive"';
	} else {
	($i % 2 == 0) ? $TR_CLASS='' : $TR_CLASS='class="even"';
}
$tpl->set_var(
array(
'TR_CLASS'		=> $TR_CLASS,
